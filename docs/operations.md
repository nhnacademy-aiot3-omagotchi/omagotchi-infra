...
- Infra 디렉터리에서 `bash tests/validate.sh` 실행
- PR·main과 같은 검증 내용, 운영 Secret·학교 서버 접속 불필요
- GitHub Runner의 SSH 준비·설정 전달 코드는 자동 배포와 수동 동기화에서 공동 사용
- 서버 배포 잠금은 Git 갱신 전 획득
  - 서버에 새 공통 파일이 없을 수 있으므로 각 배포 진입점의 잠금 함수 유지
  - 잠금 파일·대기·실패 처리의 일관성은 기존 잠금 테스트에서 확인
...
- 지원 구성
  - 준비된 A/B의 순차 교체만 지원, 기존 단일 구성의 자동 전환 기능 제거
  - 구형 단일 컨테이너가 남으면 자동 삭제 없이 중단, 실제 실행과 요청 분배 확인 필요
  - 신규 서버의 초기화와 구형 환경의 A/B 전환은 별도 절차 필요
- Frontend 정적 파일
