...
- 이번 변경: Prometheus·Grafana·Collector·Tempo 설정과 조회·증상 알림의 기본 구성
- 변경 제외: 서비스 의존성·보안 설정·업무 로직, Message Trace, 무중단 배포
- 현재 서비스 상태
...
- 순서: 배포·Filebeat Fix 운영 확인 → Infra 도구 확인 → 서비스별 계측 연결 → 실제 Dashboard·Waterfall 확인
...
- 선택 기준: 기존 계획의 제품 유지·확인된 Patch 적용, Experimental 기능 추가 없음
...
- Volume 전체의 강제 Disk Quota 없음
  - Prometheus 2GB: 보존 Block 정리 기준, WAL·Head·전체 Volume 상한 아님
...
- 보존: Trace ID·Span ID·Parent Span ID·시간·상태, 허용된 서비스 정보·HTTP 속성
- 제거: 원본 URL·Query·Header·임의 속성·Status Message·Tracestate·Span Event
- Span 이름: 검증된 HTTP Method 또는 `HTTP`, 원본 Client URL 사용 금지
- Link 포함 Span: 현재 저장 대상에서 제외
  - 선택 버전의 OTTL에서 Link 속성의 개별 정제 미지원
  - HTTP 부모·자식 경로와 Message Link의 구분, 비동기 Message 추적 도입 시 재설계
  - 해당 Span을 부모로 가진 후속 Span의 연결 단절 가능성
...
- 허용 필드 값까지 임의 비밀값을 판별하는 보안 장치 아님
  - 앱에서 URL·예외 원문 제거 우선
  - 서비스 이름·Route·Resource·Scope의 안전한 값 사용
...
- `tests/collector-privacy-test.sh`: 실제 Collector에 가짜 URL·Header·예외·Event·Link 입력 후 정제 결과 확인
- `tests/runtime-config-sync-test.sh`: Grafana 비밀번호 누락·빈 값의 거절과 기존 운영본·복구본 보존
