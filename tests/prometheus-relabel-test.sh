...
docker run --detach --name "${METRICS_NAME}" --network "${TEST_NETWORK}" \
  --network-alias prediction-service-a --network-alias prediction-service-b --read-only --memory 32m --cpus 0.5 \
...
for _ in {1..60}; do
  if docker exec "${METRICS_NAME}" wget -T 2 -qO- \
    ...
    && jq -e '[.data.result[]? | select(.metric.__name__ == "http_server_requests_seconds_count") | .metric.instance] | unique | length == 2' \
...
jq -e '
  ...
  (.data.result | map(select(.metric.__name__ | startswith("http_server_requests_seconds_"))) as $http
    | ($http | length) == 12
      and ([$http[].metric.instance] | unique | length) == 2
      and all($http[]; .metric.http_request_method == null and .metric.http_response_status_code == null
        and .metric.http_route == null)
      and ([$http[] | select(.metric.method == "POST" and .metric.status == "200" and .metric.uri == "/predict")] | length) == 6
      and ([$http[] | select(.metric.method == "GET" and .metric.status == "404" and .metric.uri == "UNMATCHED")] | length) == 6)
